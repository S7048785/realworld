...
export default function HomePage() {
  ...
  const globalFeedTab = {
    ...
    value: "Global Feed",
    content: (
      <ArticleListTabsContent
        getData={(skip: number) => api.getArticleAllList({ skip })}
      />
    ),
  };
  ...
  const yourFeedTab = {
    ...
    content: (
      <ArticleListTabsContent
        getData={(skip: number) => api.getArticleListByFollow({ skip })}
      />
    ),
  };

  const initialTabs = isAuth ? [globalFeedTab, yourFeedTab] : [globalFeedTab];
  ...
  // 当登录状态变化时，添加或移除 `Your Feed` 标签，并在必要时切换选中标签
  useEffect(() => {
    if (isAuth) {
      setTabsState((prev) => {
        if (prev.find((t) => t.value === yourFeedTab.value)) return prev;
        const next = [...prev.slice(0, 1), yourFeedTab, ...prev.slice(1)];
        return next;
      });
    } else {
      setTabsState((prev) => {
        if (!prev.find((t) => t.value === yourFeedTab.value)) return prev;
        const next = prev.filter((t) => t.value !== yourFeedTab.value);
        // 如果当前选中的是被移除的标签，切换到第一个可用标签
        if (tabValue?.value === yourFeedTab.value) {
          setTabValue(next[0] ?? globalFeedTab);
        }
        return next;
      });
    }
    // 我们在 effect 中使用 tabValue 来判断是否需要切换选中项
  }, [isAuth, tabValue]);
  ...
  return (
    <div className="h-full">
      <div className="h-40 w-full space-y-2 py-10 flex flex-col items-center justify-center bg-primary inset-shadow-[0_0_10px_rgba(0,0,0,0.5)] text-gray-200">
      ...
      <div className="pt-10 px-5 md:px-25 min-h-100 flex gap-8">
        <div className="md:w-4/5 w-full">
          <Tabs className="" value={tabValue.value}>
            <TabsList className="h-auto w-full flex justify-start gap-2 rounded-none border-b bg-transparent px-0 py-1 text-foreground text-lg">
              {tabsState.map((tab) => (
                <TabsTrigger
